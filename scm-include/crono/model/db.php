...
class DB {
    ...
    private $user = SCM_DB_USER; // USUARIO de la BASE DE DATOS
    private $password = SCM_DB_PASSWORD; // PASSWORD del usuario de la BASE DE DATOS
    private $database = SCM_DB_NAME; // BASE DE DATOS
    private $host = SCM_DB_HOST; // IPv4 o HOST DE CONEXION
    ...
    // inici de transacció contra la BBDD      
        $conx = $this->conectar();
        $conx->autocommit(FALSE);
        $conx->begin_transaction();
    }            
    public function commitTX() {               
    // commit de transacció contra la BBDD 
        $conx = $this->conectar();
        $conx->commit();
    }   
    public function rollbackTX() {        
    // rollback de la transacció contra la BBDD
        $conx = $this->conectar();
        $conx->rollback();
    }    
