...
class Table {
    ...
    /**
     * LIMIT CLAUSE SQL QUERY (quantitat de registres a tornar)
    ...
    /**
     * LIMIT CLAUSE SQL QUERY (Valor inicial)
     * @param  integer $cantidad CANTIDAD DE DATOS
     * @return Table            RETURN CURRENT CLASS
     */
    public function inici($cantidad = 1000) {
        if (is_numeric($cantidad)) {
            $this->inici = $cantidad;
        }
        return $this;
    ...
    public function getSQL() {
        ...
        if ($this->limit != '') {
            $consulta.=" LIMIT " . $this->inici . ", " . $this->limit;            
        }
        return $consulta;
    }
    public function get() {
    ...
    public function save_update($datos) {    
        ...
        if (is_object($datos) || is_array($datos)) {
            foreach ($datos as $key => $value) { 
                if ($key <> $this->KEY()) { // no inserta la clave de la tabla - para claves autogeneradas
                    if ($this->COLUMN_EXISTS(strtoupper($key))) {
                        $insert .= $key . ',';
                        $value = DB::SQL_CLEAN_TEXT($value);
                        switch ($this->COLUMN_TYPE($key)) {
        ...
        $consulta = str_replace(',)', ')', $consulta);
        $consulta =  $this->SQL_FUNCTIONS($consulta);
        $resultado = $this->mydb->ejecutar($consulta);echo $consulta;
        if ($resultado > 0) {
            return true;
        } else {
            return false;
